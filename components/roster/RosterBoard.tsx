...
type Assignment = { _id: string; date: string; roleSlotDefinitionId: string; userId: string | null; startTime?: string | null; endTime?: string | null };
...
export default function RosterBoard({ users, slots, assignments, calendar }: {
  users: User[];
  slots: Slot[];
      ...
      <div className="grid grid-cols-7 gap-1.5">
        {days.map((d) => (
          <button
            ...
            className={`rounded-lg border p-1.5 text-center text-xs ${selected === d.key ? "border-primary bg-primary/10" : "border-black/10"}`}
          >
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {DAY_TYPES.map((dt) => (
            <button
              ...
              className={`px-2.5 py-1 text-xs font-medium rounded-md ${dayType === dt.value ? "bg-primary text-white" : "bg-black/5 text-ink/60"}`}
            >
              {dt.label}
            </button>
          ))}
          <button
            ...
            className={`px-2.5 py-1 text-xs font-medium rounded-md ${selectedDay?.cal?.surgeryOverlay ? "bg-warning/20 text-warning" : "bg-black/5 text-ink/60"}`}
          >
        ...
        {visibleSlots.length === 0 ? (
          ...
          <ul className="flex flex-col divide-y divide-black/5">
            {visibleSlots.map((s) => {
              ...
              const assignedUser = a?.userId ? userById.get(a.userId) : null;
              return (
                <li key={s._id} className="py-2.5 flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {assignedUser && <Badge>{assignedUser.fullName}</Badge>}
                    <Select
                      className="w-44"
                      value={a?.userId || ""}
                      onChange={(e) => assignUser(s._id, e.target.value)}
                    >
                      <option value="">— unassigned —</option>
                      {users
                        .filter((u) => u.role === s.personType || s.personType === "intern")
                        .map((u) => (
